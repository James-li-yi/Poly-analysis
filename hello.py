...
if df_2024 is not None and df_2025 is not None:
    ...
    with col1:
        ...
        for i, platform in enumerate(pivot_data.columns):
            fig.add_trace(go.Bar(
                ...
                text=[f'{platform}<br>{val:.1f}万<br>({pivot_percentage.loc[year, platform]:.1f}%)' 
                    for year, val in zip(pivot_data.index, pivot_data[platform])],
                textposition='inside',
                textfont=dict(size=10, color='white'),
                hovertemplate='<b>%{fullData.name}</b><br>年份: %{x}<br>业绩金额: %{y:.2f}万<br>占比: %{customdata:.1f}%<extra></extra>',
    ...
    if key_cities_data:
        ...
        fig3 = px.bar(
            ...
            title="重点城市业绩增长情况",
            labels={'x': '城市', 'y': '增长金额'},
            color=key_cities_df['增长额'].tolist(),
            color_continuous_scale='RdYlGn'
        )
        ...
        # # 显示重点城市详细数据
        # st.write("**重点城市详细数据:**")
        # for _, row in key_cities_df.iterrows():
        #     st.write(f"{row['城市']}: {row['增长额']:,.0f}")
    else:
        st.write("重点城市均无业绩数据")
    ...
    fig4.add_trace(go.Bar(
        ...
        name='增长量',
        marker_color=['green' if x >= 0 else 'red' for x in format_growth_sorted.values],
        yaxis='y'
    ))
    ...
    fig4.add_trace(go.Scatter(
        x=format_growth_sorted.index.tolist(),
        y=format_growth_rate_sorted.values.tolist(),
        mode='lines+markers',
        name='增长率(%)',
    ...
    #     text=city_df['增长率'].apply(lambda x: f'{x:.1f}%'),
    #     textposition='top center',
    ...
    if len(city_df) == 0:
        ...
        fig.add_trace(go.Scatter(
            ...
            y=city_df['增长率'],
            mode='lines+markers',
            marker=dict(size=8, color='#6771E2'),
            line=dict(color='#6771E2', width=3),
            text=city_df['增长率'].apply(lambda x: f'{x:.1f}%'),
            textposition='top center',
        ...
        # 构建标题，包含没有数据的城市信息
        chart_title = '重点城市业绩分析：双柱状图 + 增长率折线图'
        if cities_without_data:
            chart_title += f'<br><sub>无业绩数据的重点城市：{", ".join(cities_without_data)}</sub>'
        # 更新布局 - 调整为深色主题
        # 更新布局 - 调整为深色主题
        fig.update_layout(
        title=chart_title,
    ...
    if len(ranking_df) > 0:
        ...
        # 为每个城市添加连接线
        for i, row in ranking_df.iterrows():
            if row['2024年排名'] is not None and row['2025年排名'] is not None:
                fig.add_trace(go.Scatter(
                    x=[row['2024年排名'], row['2025年排名']],
                    y=[row['城市'], row['城市']],
                    mode='lines',
        ...
        # 添加2024年数据点
        cities_2024 = ranking_df[ranking_df['2024年排名'].notna()]
        if len(cities_2024) > 0:
            fig.add_trace(go.Scatter(
                x=cities_2024['2024年排名'],
                y=cities_2024['城市'],
                mode='markers',
                marker=dict(
                    color='#3498db',
                ...
                name='2024年排名',
                text=cities_2024.apply(lambda row: f"{row['城市']}<br>2024年排名: {row['2024年排名']}<br>业绩: {row['2024年业绩']:,.0f}", axis=1),
                hovertemplate='%{text}<extra></extra>'
            ))

        # 添加2025年数据点
        cities_2025 = ranking_df[ranking_df['2025年排名'].notna()]
        if len(cities_2025) > 0:
            fig.add_trace(go.Scatter(
                x=cities_2025['2025年排名'],
                y=cities_2025['城市'],
                mode='markers',
                marker=dict(
                    color='#e74c3c',
                ...
                name='2025年排名',
                text=cities_2025.apply(lambda row: f"{row['城市']}<br>2025年排名: {row['2025年排名']}<br>业绩: {row['2025年业绩']:,.0f}", axis=1),
                hovertemplate='%{text}<extra></extra>'
            ))
...
# 如果没有年份列，需要先添加年份信息
    ...
    fig.add_trace(
        go.Scatter(
            name='增长率',
            x=industry_pivot_sorted.index,
            y=industry_pivot_sorted['增长率'],
            mode='lines+markers',
            line=dict(color='#6771E2', width=3),  # 深蓝色，与PPT背景协调
            marker=dict(size=8, color='#6771E2'),
            text=industry_pivot_sorted['增长率'],
            texttemplate='%{text:.1f}%',
            textposition='top center',
            textfont=dict(size=12, color='#1B4965')  # 深蓝色文字，确保清晰可见
        ),
        secondary_y=True,
    )
    ...
    fig8.update_layout(yaxis={'categoryorder':'total ascending'},plot_bgcolor='#E3EAF3', 
    paper_bgcolor='#E3EAF3',font=dict(color='#1B4965', size=12),  # 全局字体颜色
    title_font=dict(color='#1B4965', size=16),  # 标题单独设置
    xaxis=dict(tickfont=dict(color='#1B4965', size=12)) # X轴刻度标签
    )
    fig8.update_layout(yaxis=dict(tickfont=dict(color='#1B4965', size=12)))
